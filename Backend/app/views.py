...
@api_view(['POST'])
def toggle_block_profile(request):
    try:
        # Unlike the previous version, this won't require authentication
        username = request.data.get('username')
        social_type = request.data.get('social')

        if not username or not social_type:
            return Response({'error': 'Username and social platform are required'},
                            status=status.HTTP_400_BAD_REQUEST)

        try:
            profile = Profile.objects.get(username=username, social__social=social_type)
        except Profile.DoesNotExist:
            return Response({'error': 'Profile not found'},
                            status=status.HTTP_404_NOT_FOUND)

        # Since no authentication is required, we'll create a default settings
        ...
        # Check if profile is already blocked
